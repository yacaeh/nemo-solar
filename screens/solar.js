...
export default function ({ navigation }) {
  ...
  function getSystemList(querySnapshot) {
    ...
    querySnapshot.forEach((res) => {
      const { id, sunvoltage, sunvoltage3, sunvoltag4, pcbsignal } = res.data();
      systemList = systemList.concat({
        ...
        pcbsignal: pcbsignal ? pcbsignal : false,
      });
    });
    setLoading(false);
  ...
  const ItemContainer = (props) => (
    ...
    >
      ...
      >
        <Item
          title="태양광1"
          content={props.system.sunvoltage + ' V (전앖값)' || '데이터 없음'}
        />
        <Item
          title="태양광2"
          content={props.system.sunvoltage + ' V (전앖값)' || '데이터 없음'}
        ...
        <View style={{ top:10,flex: 1, height: 50, flexDirection: 'row' }}>
          <View style={{ flex: 8 }}>
            <Text
              style={{
                fontSize: 20,
          ...
          <View style={{ top:-5, flex: 2, height: 50 }}>
            {props.system.pcbsignal ? (
              <Image
